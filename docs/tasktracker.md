...
| 10. Промпты для AI провайдеров | 1 | 1 | 0 | 0 |
| **Итого** | **19** | **19** | **0** | **0** |
...
## Этап 10: Промпты для AI провайдеров

### Задача 10.1: Экран настроек провайдера с полем промпта
- **Приоритет**: Высокий
- **Статус**: Завершена
- **Описание**: Добавлен экран настроек AI провайдера с полем ввода промпта. Промпт склеивается с распознанным текстом перед отправкой в AI. Каждый провайдер (z.ai, Claude) имеет свой отдельный промпт.
- **Шаги выполнения**:
  - [x] PreferencesStore: добавлены ключи zaiPrompt, claudePrompt
  - [x] AiProviderViewModel: управление промптом для конкретного провайдера
  - [x] AiProviderScreen: UI с многострочным полем ввода промпта, кнопками сохранить/очистить
  - [x] AppNavigation: маршрут ai_provider/{providerKey}, скрытие bottom bar
  - [x] MainScreen: кнопки провайдеров навигируют на экран настроек
  - [x] VoiceForegroundService: чтение промпта из DataStore, склейка с распознанным текстом
  - [x] BUILD SUCCESSFUL
- **Зависимости**: Этап 9
